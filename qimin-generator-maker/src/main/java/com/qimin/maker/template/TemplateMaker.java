...
import com.qimin.maker.template.model.TemplateMakerFileConfig;
import com.qimin.maker.template.model.TemplateMakerModelConfig;
...
public class TemplateMaker {
    ...
    public static void main(String[] args) {
        ...
        String originProjectPath = new File(projectPath).getParent() + File.separator + "qimin-generator-demo-projects/springboot-init-master";
        ...
        // 模型组配置
        TemplateMakerModelConfig.ModelGroupConfig modelInfoConfig = new TemplateMakerModelConfig.ModelGroupConfig();
        modelInfoConfig.setGroupKey("mysql");
        modelInfoConfig.setGroupName("数据库配置");
        templateMakerModelConfig.setModelGroupConfig(modelInfoConfig);
...
//        String searchStr = "Sum:";
        ...
        long id = makeTemplate(meta, originProjectPath, templateMakerFileConfig, templateMakerModelConfig, 1875070470539390976L);
    ...
    private static long makeTemplate(Meta newMeta, String originProjectPath, TemplateMakerFileConfig templateMakerFileConfig, TemplateMakerModelConfig templateMakerModelConfig, Long id) {
        ...
        // - 转化为配置接受的 modelInfo 对象
        List<Meta.ModelConfig.ModelsInfo> inputModelsInfoList = models.stream().map(modelInfoConfig -> {
            Meta.ModelConfig.ModelsInfo modelsInfo = new Meta.ModelConfig.ModelsInfo();
        ...
        // - 本次新增的模型配置列表
        List<Meta.ModelConfig.ModelsInfo> newModelsInfoList = new ArrayList<>();
        ...
        if (modelGroupConfig != null) {
            ...
            Meta.ModelConfig.ModelsInfo groupModelsInfo = new Meta.ModelConfig.ModelsInfo();
            groupModelsInfo.setCondition(condition);
            groupModelsInfo.setGroupKey(groupKey);
            groupModelsInfo.setGroupName(groupName);

            // 模型全放在一个分组内
            groupModelsInfo.setModels(inputModelsInfoList);
            newModelsInfoList.add(groupModelsInfo);
        }else {
            // 不分组，添加所有的模型信息到列表
            newModelsInfoList.addAll(inputModelsInfoList);
        }
        ...
        if (FileUtil.exist(metaOutputPath)) {
            ...
            modelsInfoList.addAll(newModelsInfoList);
        }
    ...
    private static List<Meta.ModelConfig.ModelsInfo> distinctModels(List<Meta.ModelConfig.ModelsInfo> modelInfoList) {
        ...
        //1. 有分组的，以组为单位划分
        Map<String, List<Meta.ModelConfig.ModelsInfo>> groupKeyModelInfoListMap = modelInfoList.stream()
                .filter(modelInfo -> StrUtil.isNotBlank(modelInfo.getGroupKey()))
                .collect(
                        Collectors.groupingBy(Meta.ModelConfig.ModelsInfo::getGroupKey)
                );

        // 2. 同组内的模型配置合并
        // 保证每个组对应的合并后的对象map
        Map<String, Meta.ModelConfig.ModelsInfo> groupKeyMergedModelInfoMap = new HashMap<>();
        for (Map.Entry<String, List<Meta.ModelConfig.ModelsInfo>> entry : groupKeyModelInfoListMap.entrySet()) {
            List<Meta.ModelConfig.ModelsInfo> tempModelInfoList = entry.getValue();
            List<Meta.ModelConfig.ModelsInfo> newModelInfoList = new ArrayList<>(tempModelInfoList.stream()
                    .flatMap(modelInfo -> modelInfo.getModels().stream())
                    .collect(Collectors.toMap(Meta.ModelConfig.ModelsInfo::getFieldName, o -> o, (e, r) -> r)
                    ).values());

            // 使用新的 group 配置
            Meta.ModelConfig.ModelsInfo newModelInfo = CollUtil.getLast(tempModelInfoList);
        ...
        // 3. 将模型分组添加到结果列表
        List<Meta.ModelConfig.ModelsInfo> resultList = new ArrayList<>(groupKeyMergedModelInfoMap.values());

        // 4. 将为分组的模型添加到结果列表
        List<Meta.ModelConfig.ModelsInfo> noGroupModelInfoList = modelInfoList.stream().filter(modelInfo -> StrUtil.isBlank(modelInfo.getGroupKey()))
                .collect(Collectors.toList());
        resultList.addAll(new ArrayList<>(noGroupModelInfoList.stream()
                .collect(
                        Collectors.toMap(Meta.ModelConfig.ModelsInfo::getFieldName, o -> o, (e, r) -> r)
                ).values()));
        return resultList;
    }
}
